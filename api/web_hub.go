...
func shouldSendEvent(webCon *WebConn, msg *model.WebSocketEvent) bool {
	if webCon.UserId == msg.UserId {
		...
		if msg.Event == model.EVENT_TYPING {
			return false
		}
		...
		if msg.Event == model.EVENT_CHANNEL_VIEWED {
			return false
		} else if msg.Event == model.EVENT_PREFERENCE_CHANGED {
			return false
		} else if msg.Event == model.EVENT_EPHEMERAL_MESSAGE {
		...
		if len(msg.ChannelId) > 0 && msg.Event != model.EVENT_CHANNEL_DELETED {
			allowed := webCon.HasPermissionsToChannel(msg.ChannelId)
