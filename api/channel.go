...
func CreateDirectChannel(userId string, otherUserId string) (*model.Channel, *model.AppError) {
	...
	if result := <-Srv.Store.Channel().SaveDirectChannel(channel, cm1, cm2); result.Err != nil {
		...
		message := model.NewWebSocketEvent("", channel.Id, userId, model.EVENT_DIRECT_ADDED)
...
func AddUserToChannel(user *model.User, channel *model.Channel) (*model.ChannelMember, *model.AppError) {
	...
	go func() {
		...
		message := model.NewWebSocketEvent(channel.TeamId, channel.Id, user.Id, model.EVENT_USER_ADDED)
		go Publish(message)
	}()
...
func deleteChannel(c *Context, w http.ResponseWriter, r *http.Request) {
	...
	if cresult := <-sc; cresult.Err != nil {
		...
		go func() {
			...
			message := model.NewWebSocketEvent(c.TeamId, channel.Id, c.Session.UserId, model.EVENT_CHANNEL_DELETED)
			go Publish(message)
...
func updateLastViewedAt(c *Context, w http.ResponseWriter, r *http.Request) {
	...
	message := model.NewWebSocketEvent(c.TeamId, id, c.Session.UserId, model.EVENT_CHANNEL_VIEWED)
...
func RemoveUserFromChannel(userIdToRemove string, removerUserId string, channel *model.Channel) *model.AppError {
	...
	message := model.NewWebSocketEvent(channel.TeamId, channel.Id, userIdToRemove, model.EVENT_USER_REMOVED)
